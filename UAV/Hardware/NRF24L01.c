...
#define NRF24L01_TX_PACKET_WIDTH		8							//发送数据包宽度，范围：1~32字节
...
#define NRF24L01_RX_PACKET_WIDTH		8							//接收通道0数据包宽度，范围：1~32字节
...
uint8_t NRF24L01_R_MISO(void)
{
    return (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_14) == GPIO_PIN_SET) ? 1 : 0;
}
...
void NRF24L01_GPIO_Init(void)
{
    ...
    GPIO_InitStruct.Pin = GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_13 | GPIO_PIN_15;
    ...
    // GPIO_InitStruct.Speed 不需要设置为输入模式
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

    // 设置引脚初始化后的默认电平
    NRF24L01_W_CE(0);    // CE默认为0
    NRF24L01_W_CSN(1);   // CSN默认为1
    NRF24L01_W_SCK(0);   // SCK默认为0
    NRF24L01_W_MOSI(0);  // MOSI默认电平随意
}
/*********************引脚配置*/
